...
INCLUDE_KW = [
    ...
    "export control","CHIPS Act","subsidy","restriction",
...
def analyze(articles):
    ...
    prompt = f"""당신은 글로벌 반도체 산업 전문 리서치 애널리스트입니다.
아래 기사 목록을 분석하여 산업 실질 데이터와 기술적 사실이 포함된 기사를 중심으로 인사이트를 작성하세요.
...
{{
  ...
  "stories": [
    {{
      "id":       "원본 기사 id",
      "title_ko": "한국어 제목 (영어면 번역)",
      "company":  "주요 관련 기업",
      "topic":    "파운드리|메모리|AI칩|장비·소재|정책·규제|투자·M&A|공급망|기타 중 하나",
      "region":   "한국|미국|대만|중국|일본|유럽|글로벌 중 하나",
      "insight":  "산업적 의미, 수치·데이터 포함 2~3문장",
      "impact":   1
    }}
  ]
}}

stories는 영향도 높은 순으로 정렬하되 건수 제한 없이 전부 포함하세요."""
...
def run(days_back=1):
    ...
    if not articles:
        print("  수집된 기사가 없습니다.")
        return
    print(f"[2/3] Gemini 분석 중... ({len(articles)}건)")
